...
ExTester supports a **floating window** of the 3 most recent VS Code stable releases:

```
Example:
┌─────────────────────────────────────────┐
│  Min: 1.109.5  (oldest supported)       │
│  Middle: 1.110.1  (previous release)    │
│  Max: 1.111.0  (latest supported)       │
└─────────────────────────────────────────┘
```

When a new VS Code version is released, the window shifts:

```
New release: 1.112.0
┌─────────────────────────────────────────┐
│  Min: 1.110.1  (was middle)             │
│  Middle: 1.111.0  (was max)             │
│  Max: 1.112.0  (new release)            │
└─────────────────────────────────────────┘
```
...
2. **Fetches** the latest 3 stable VS Code versions from the official API
3. **Compares** with current versions in the repository
4. **Updates** if changes are detected:
...
6. **Triggers CI tests** against all 3 versions automatically
